...
const request = async (url:string,data:any,token:string | null | undefined) => {
...
export const post = (url:string, body:any,token?:string) => request(`${BASE_URL}${url}`, {method:"POST",body},token);

export const put = (url:string, body:any,token?:string) => request(`${BASE_URL}${url}`, {method:"PUT", body}, token);
